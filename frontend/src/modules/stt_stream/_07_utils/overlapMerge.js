/**
 * Overlap Merge Utility
 * pending interim과 새 final 결과의 오버랩 병합
 */

import { normalizeForComparison } from './textFormatter'
...
export function overlapMerge(pending, firstFinal) {
  ...
  if (normPending === normFinal) {
    debugStore.info('overlapMerge: exact duplicate, using firstFinal', { pending, firstFinal })
    return firstFinal
  }

  // pending이 firstFinal에 포함되어 있으면 firstFinal만 사용
  if (normFinal.includes(normPending)) {
    debugStore.info('overlapMerge: pending contained in firstFinal', { pending, firstFinal })
    return firstFinal
  }

  // firstFinal이 pending에 포함되어 있으면 pending만 사용
  if (normPending.includes(normFinal)) {
    debugStore.info('overlapMerge: firstFinal contained in pending', { pending, firstFinal })
    return pending
  }
  ...
  for (let overlapLen = maxOverlap; overlapLen >= 1; overlapLen--) {
    ...
    if (pendingTail === finalHead) {
      ...
      debugStore.info('overlapMerge: partial overlap found', { 
        pending, 
        firstFinal, 
        overlapLen, 
        merged 
  ...
  // 오버랩 없음 - 둘 다 사용
  debugStore.info('overlapMerge: no overlap, concatenating', { pending, firstFinal })
