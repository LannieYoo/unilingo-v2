...
class STTService:
    ...
    LANG_MAP = {
        'ko': 'ko', 'en-CA': 'en', 'en-US': 'en', 'en-GB': 'en', 'en-IN': 'en',
        'zh': 'zh', 'auto': None, '': None
    }
    ...
    def verify_dependencies(self) -> Dict[str, Dict]:
        """의존성 확인"""
        dependencies = {}
    ...
    def transcribe(self, audio_file: FileStorage, language: Optional[str] = None, trace_id: Optional[str] = None) -> Dict[str, Any]:
        """오디오를 텍스트로 변환"""
        if not self.model:
            raise RuntimeError("STT model not loaded")
        
        processing_details = {'ffmpeg_available': self.ffmpeg_path is not None, 'audio_conversion': 'none', 'trace_id': trace_id}
        ...
        if file_size < 200:
            ...
            return {'text': '', 'language': language or 'en', 'filtered': True, 'confidence': 0.0, 'fallback_used': False, 'processing_details': {'skipped': 'file_too_small', 'file_size': file_size, 'trace_id': trace_id}}
        ...
        for path in ffmpeg_paths:
            try:
                ...
                if result.returncode == 0:
                    ...
                    return {'name': 'ffmpeg', 'required': False, 'available': True, 'version': version}
            except Exception:
                continue
        return {'name': 'ffmpeg', 'required': False, 'available': False, 'installation_guide': self._get_ffmpeg_installation_guide(), 'error_details': 'FFmpeg not found'}
    ...
    def _find_ffmpeg_paths(self) -> List[str]:
        ...
        if system == 'windows':
            ...
            paths.extend([r'C:\ffmpeg\bin\ffmpeg.exe', r'C:\Program Files\ffmpeg\bin\ffmpeg.exe', rf'C:\Users\{username}\scoop\apps\ffmpeg\current\bin\ffmpeg.exe'])
        elif system == 'darwin':
    ...
    def _check_faster_whisper(self) -> Dict:
        """Faster-Whisper 확인"""
        if FASTER_WHISPER_AVAILABLE:
            try:
                import faster_whisper
                version = getattr(faster_whisper, '__version__', 'unknown')
                return {'name': 'faster_whisper', 'required': True, 'available': True, 'version': version}
            except Exception as e:
                return {'name': 'faster_whisper', 'required': True, 'available': False, 'error_details': str(e), 'installation_guide': 'pip install faster-whisper'}
        return {'name': 'faster_whisper', 'required': True, 'available': False, 'error_details': 'Not imported', 'installation_guide': 'pip install faster-whisper'}
    ...
    def _convert_to_wav(self, input_path: str) -> str:
        ...
        if not self.ffmpeg_path:
            return input_path
        try:
            cmd = [self.ffmpeg_path, '-i', input_path, '-ar', '16000', '-ac', '1', '-acodec', 'pcm_s16le', '-y', wav_path]
    ...
    def _transcribe_audio(self, audio_path: str, language: Optional[str], trace_id: Optional[str] = None) -> Dict[str, Any]:
        """음성인식 실행"""
        transcribe_options = {
            'beam_size': 5, 'best_of': 5, 'temperature': 0.0, 'condition_on_previous_text': False,
            'vad_filter': True, 'vad_parameters': {'threshold': 0.5, 'min_speech_duration_ms': 250, 'min_silence_duration_ms': 500},
        ...
        if is_hallucination:
            ...
            text = ''
        return {'text': text, 'language': detected_language, 'filtered': is_hallucination, 'confidence': info.language_probability if hasattr(info, 'language_probability') else 0.0, 'fallback_used': False, 'processing_details': {}}
...
def get_stt_service(model_name: str = 'medium', device: str = 'cpu', compute_type: str = 'int8') -> STTService:
    """전역 STT 서비스 인스턴스 반환"""
    global _stt_service
...
def get_dependency_status() -> Dict[str, Dict]:
    ...
    try:
        ...
        return {'error': {'name': 'system', 'required': True, 'available': False, 'error_details': str(e)}}
